...
- `physical` 方案的 APP 显示器目标在面板 GPU 路径切换后按物理身份映射到在线路径（唯一孪生 ID）；映射缺失或有歧义时才回退运行时主屏，绝不空等旧 GPU 路径 ID（否则会形成探测/捕获跨适配器错配，如 QSV 探测 + NVIDIA 采集）。
...
- `physical` 目标（APP 显式显示器或全局输出）在路径切换后：旧 ID 不在枚举（或仅 INACTIVE 残留）且存在唯一在线孪生（相同 `physical_identity`）→ 解析意图映射到在线路径；捕获端（RTSP monitor）跟随解析后的意图而非客户端/应用缓存的旧 ID，探测与采集保持同一 GPU 路径。
- 客户端/应用缓存的已销毁 VDD ID（`SUNSHINE_CLIENT_DISPLAY_NAME`）不会拖走 `physical` 目标会话：`display_target_override == 0` 时忽略客户端环境变量并解析到运行时主屏。
