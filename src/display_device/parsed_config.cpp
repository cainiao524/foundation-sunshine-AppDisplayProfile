...
#undef _CONVERT_
  ...
  make_parsed_config(
    ...
    // 客户端（如 Moonlight V+）可能发送 no_operation 显示模式，全局 ensure_active
    // 映射到 VDD 时也是 no_operation。此时尊重基地版 no_operation 语义：不激活
    // VDD、不改变显示拓扑，直接串流当前活动显示器（物理主屏），与基地原版行为一致。
